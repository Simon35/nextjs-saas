...
const InvoiceComponent: React.FC<InvoiceComponentProps> = ({ invoice }) => {
    ...
    const calculateTotal = () => {
        // on fait le calcul totalHT si invoice contient des lignes
        // la InvoiceLine est comme un produit
        // on précise acc: 0
        const totalHT = invoice?.lines?.reduce((acc, line) => {
            const quantity = line.quantity ?? 0; // so on n'a pas de qté, égal à 0
    ...
    return (
        <div className='bg-base-200/90 p-5 rounded-xl space-y-2 shadow'>
            ...
            <div className='w-full'>
                <div >
                    <div className='stat-title'> {/*daisy ui*/}
                        <div className='uppercase text-sm'>FACT-{invoice.id}</div>
                    </div>
                    <div>
                        <div className='stat-value'>  {/* toFixed pour les cts */}
